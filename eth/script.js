...
// CoinGecko API端点：专属 Ethereum Ecosystem 中的 MEME 币，按市值排序前20
const API_URL = 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=ethereum-ecosystem&order=market_cap_desc&per_page=20&page=1&sparkline=false&price_change_percentage=24h';

// 流行交易所映射：针对 Ethereum meme 币优化 CEX 和 DEX
const EXCHANGE_MAP = {
  cex: [
  ...
  dex: [
    { id: 'uniswap_v3', name: 'Uniswap (ETH DEX)', url: 'https://app.uniswap.org/swap' },
    { id: 'sushiswap', name: 'SushiSwap (ETH DEX)', url: 'https://app.sushi.com/swap' }
  ]
};
...
async function getExchangeRecommendations(coinId, symbol) {
  try {
    ...
    // 以太坊 DEX 推荐：优先 Uniswap，其次 SushiSwap
    const dexRec = EXCHANGE_MAP.dex[0]; // 默认 Uniswap
    ...
    // 添加 DEX（以太坊特定：使用 Uniswap swap 链接，需 token 地址）
    const tokenAddress = await getTokenAddress(coinId);
    recommendations.push({
      type: 'dex',
      ...dexRec,
      link: `${dexRec.url}?outputCurrency=${tokenAddress || '0x...'}&chain=eth` // ETH 作为 input，token 作为 output
    });
    ...
    return [
      ...
      { type: 'dex', name: 'Uniswap', url: 'https://app.uniswap.org/swap', link: 'https://app.uniswap.org/swap' }
    ];
  }
}

// 获取以太坊 token 地址（从 CoinGecko /coins/{id}）
async function getTokenAddress(coinId) {
  try {
    const response = await fetch(`https://api.coingecko.com/api/v3/coins/${coinId}`);
    const data = await response.json();
    return data.platforms?.ethereum || '0x...'; // 以太坊合约地址
  } catch {
    return '0x...'; // 占位
  }
}
...
async function loadData() {
    try {
        ...
        // 额外过滤：仅保留 meme 主题（基于 CoinGecko 标签或名称关键词，如 'meme'、'shiba'、'pepe' 等）
        data = data.filter(coin => coin.name.toLowerCase().includes('meme') || coin.id.includes('shiba-inu') || coin.id.includes('pepe') || coin.id.includes('mog-coin') || coin.categories?.includes('Meme'));
...
async function renderList(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        li.innerHTML = `
            ...
            <img src="${coin.image}" alt="以太坊 Meme币 ${coin.name} 市值排名 ${i + 1}" class="meme-img" loading="lazy">
            ...
            <div class="exchange-section">
                <div class="exchange-title">推荐交易所 (以太坊支持)</div>
