...
// CoinGecko API端点：专属 Base Ecosystem 中的 MEME 币，按市值排序前20
const API_URL = 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&category=base-ecosystem&order=market_cap_desc&per_page=20&page=1&sparkline=false&price_change_percentage=24h';

// 流行交易所映射：针对 Base meme 币优化 CEX 和 DEX
const EXCHANGE_MAP = {
  cex: [
  ...
  dex: [
    { id: 'aerodrome', name: 'Aerodrome (Base DEX)', url: 'https://aerodrome.finance/swap' },
    { id: 'uniswap_v3', name: 'Uniswap (Base L2 DEX)', url: 'https://app.uniswap.org/swap?chain=base' }
  ]
};
...
async function getExchangeRecommendations(coinId, symbol) {
  try {
    ...
    // Base DEX 推荐：优先 Aerodrome，其次 Uniswap L2
    const dexRec = EXCHANGE_MAP.dex[0]; // 默认 Aerodrome
    ...
    // 添加 DEX（Base 特定：使用 Aerodrome swap 链接，需 token 地址）
    const tokenAddress = await getTokenAddress(coinId);
    recommendations.push({
      type: 'dex',
      ...dexRec,
      link: `${dexRec.url}?outputCurrency=${tokenAddress || '0x...'}&chain=base` // ETH/Base 作为 input，token 作为 output
    });
    ...
    return [
      ...
      { type: 'dex', name: 'Aerodrome', url: 'https://aerodrome.finance/swap', link: 'https://aerodrome.finance/swap' }
    ];
  }
}

// 获取 Base token 地址（从 CoinGecko /coins/{id}）
async function getTokenAddress(coinId) {
  try {
    const response = await fetch(`https://api.coingecko.com/api/v3/coins/${coinId}`);
    const data = await response.json();
    return data.platforms?.base || '0x...'; // Base 合约地址
  } catch {
    return '0x...'; // 占位
  }
}
...
async function loadData() {
    try {
        ...
        // 额外过滤：仅保留 meme 主题（基于 CoinGecko 标签或名称关键词，如 'meme'、'brett'、'toshi' 等）
        data = data.filter(coin => coin.name.toLowerCase().includes('meme') || coin.id.includes('brett') || coin.id.includes('toshi') || coin.id.includes('degen-base') || coin.id.includes('migu') || coin.categories?.includes('Meme'));
...
async function renderList(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        li.innerHTML = `
            ...
            <img src="${coin.image}" alt="Base Meme币 ${coin.name} 市值排名 ${i + 1}" class="meme-img" loading="lazy">
            ...
            <div class="exchange-section">
                <div class="exchange-title">推荐交易所 (Base 支持)</div>
